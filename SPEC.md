...
- タッチ端末(pointer: coarse)ではキャンバス下に可視の操作バー(左回転・推力・右回転の 3 ボタン)を
  表示し、同じ操作を提供する。マルチタッチで推力と回転の同時押しができる(REQ-010)
- タイトル/リザルト画面では Enter に加えタップ/クリックでも開始・再開できる(タッチ端末対応)
- タイトル画面下部にフリート統一書式のライセンス表記「MIT License © 2026 坂田哲朗 ・ GitHub」を表示する(「MIT License」は LICENSE 本文へ、「GitHub」はソースリポジトリへのリンク)(REQ-009)
