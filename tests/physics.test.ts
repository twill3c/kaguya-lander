...
describe("Loop 1: physics.step", () => {
  it.fails("T1-1 自由落下: 1 秒後 vy ≈ GRAVITY、姿勢は直立のまま", () => {
  ...
  it.fails("T1-2 直立全推力: 1 秒後 vy ≈ GRAVITY - THRUST_ACCEL", () => {
    const s = run(initialState(), burn, 1);
    expect(s.vy).toBeCloseTo(GRAVITY - THRUST_ACCEL, 1);
  });

  it.fails("T1-3 燃料消費: 全推力 1 秒で -10、0 未満にならない", () => {
    const s = run(initialState(), burn, 1);
  ...
  it.fails("T1-4 燃料切れで推力無効: 自由落下と同一軌道", () => {
  ...
  it.fails("T1-5 回転: rotate=1 を 0.5 秒で angle ≈ +45", () => {
  ...
  it.fails("T1-6 純関数性: 引数 state を破壊しない", () => {
  ...
  it.fails("T1-7 傾斜推力の分解: angle=90 で水平方向に加速", () => {
